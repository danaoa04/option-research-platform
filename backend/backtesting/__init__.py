"""Backtesting package exports for deterministic historical replay."""

from .analytics import (
    BacktestAnalyticsService,
    ComparisonResult,
    ConversionAnalyticsResult,
    DrawdownPoint,
    GreeksAttribution,
    PerformanceAnalytics,
    PnLAttribution,
    PortfolioAnalyticsPoint,
    RollAnalyticsResult,
    StrategyAnalyticsPoint,
)
from .api_contracts import (
    ApiEnvelopeV1,
    AttributionContractV1,
    CalendarRollAnalysisContractV1,
    ComparisonContractV1,
    ConversionPreviewContractV1,
    CurveContractV1,
    CustomStrategyCreationContractV1,
    HistoryContractV1,
    ManagementComparisonContractV1,
    PMCCRollAnalysisContractV1,
    PortfolioConflictContractV1,
    ReplayControlContractV1,
    ReplayRollEventContractV1,
    ReplaySnapshotContractV1,
    RollEligibilityContractV1,
    RollHistoryContractV1,
    RollPlanContractV1,
    RollPolicyCatalogueContractV1,
    RollPreviewContractV1,
    RollRequestContractV1,
    RunSummaryContractV1,
    StrategyCycleContractV1,
    StrategyOptimizerCompatibilityContractV1,
    StrategyParameterSchemaContractV1,
    StrategyPayoffPreviewContractV1,
    StrategyPolicyCatalogueContractV1,
    StrategyPolicyConflictContractV1,
    StrategyPolicyEvaluationContractV1,
    StrategyPolicySetContractV1,
    StrategyRiskClassificationContractV1,
    StrategyTemplateCatalogueContractV1,
    StrategyTemplateDetailContractV1,
    StrategyValidationContractV1,
    TradeListContractV1,
)
from .arbitration import (
    ArbitrationDecision,
    ArbitrationPolicy,
    CompetingAction,
    CrossStrategyArbitrator,
)
from .benchmarks import BacktestBenchmarkResult, BacktestBenchmarkRunner
from .clock import HistoricalEventClock
from .engine import BacktestingEngine
from .event_taxonomy import (
    OverlayMetrics,
    RichEvent,
    RichEventPriority,
    RichEventType,
    default_priority,
)
from .exceptions import (
    BacktestingError,
    EventClockError,
    FillModelError,
    NoLookAheadError,
    StrategyLifecycleError,
    ValuationError,
)
from .execution import (
    ExecutionAction,
    ExecutionOrderType,
    ExecutionRequest,
    ExecutionSide,
    ExerciseAssignmentOrchestrator,
    FillAttempt,
    MultiLegExecutionCoordinator,
    MultiLegExecutionResult,
    OpenCloseEffect,
    execution_reproducibility_checksum,
)
from .execution_calibration import (
    BacktestExecutionPolicySelection,
    BrokerPolicyComparisonEngine,
    BrokerPolicyRegistry,
    CalibrationResult,
    CalibrationValidationRun,
    CalibrationValidator,
    ExecutionCalibrationRecord,
    ExecutionQualityScore,
    ExecutionQualityScorer,
    ExecutionSourceType,
    ExecutionStressResult,
    ExecutionStressScenario,
    ExecutionStressTestEngine,
    FillExpectation,
    FillQualityAggregate,
    FillQualityAnalyzer,
    FillQualityMetrics,
    GenericBaselineBrokerPolicy,
    InteractiveBrokersStyleResearchPolicy,
    LiquidityRegime,
    MarketRegime,
    MultiLegExecutionRealism,
    MultiLegExecutionRealismAnalyzer,
    PartialFillCalibrationResult,
    PartialFillCalibrator,
    PlaceholderMarketImpactModel,
    PolicyComparisonResult,
    RealVsSimulatedComparator,
    RealVsSimulatedFillComparison,
    ReplayExecutionInspection,
    SchwabThinkorswimStyleResearchPolicy,
    SlippageCalibrator,
    SlippageModelKind,
    SpreadCaptureCalibrator,
    StrategyExecutionProfile,
    TastytradeStyleResearchPolicy,
    TimeOfDayBucket,
    TransactionCostBreakdown,
    TransactionCostEngine,
    UserDefinedBrokerPolicy,
    VolatilityRegime,
    classify_liquidity_regime,
    classify_time_of_day,
    default_execution_stress_scenarios,
    default_strategy_execution_profiles,
    execution_calibration_checksum,
)
from .execution_queries import ExecutionQueryService, QueryAsOfResult
from .fees import FeeItem, FeeModelRequest, ItemizedFeeModel
from .fill_models import FillModelRequest, FillModelResult, ResearchFillModelEngine
from .fills import BaselineResearchFillModel, FillModelConfig
from .guards import NoLookAheadGuard
from .margining import MarginEvaluationSnapshot, MarginLifecycleCoordinator, aware_timestamp
from .models import (
    BacktestConfiguration,
    BacktestRunResult,
    BacktestStatus,
    ClockEvent,
    DeterministicEvent,
    EventClockConfig,
    EventType,
    FillPricePolicy,
    MarkPricePolicy,
    OrderAction,
    OrderIntent,
    OrderSide,
    PortfolioSnapshot,
    ScenarioTemplate,
    StrategyLifecycle,
    TradingSession,
)
from .orchestration import (
    CoordinatedEntryPlan,
    EntryMode,
    FillReconciliationEvent,
    IntegrityFailure,
    LegFillSnapshot,
    MultiLegOrchestrator,
    RollEligibilityResult,
    RollKind,
    RollPlan,
)
from .policies import (
    ConflictMode,
    ConflictResolutionResult,
    LifecyclePolicySignal,
    PolicyConflictResolver,
    PolicyDisposition,
)
from .queries import AsOfResult, BacktestAsOfQueryService
from .quote_selection import (
    QuotePriceSelection,
    QuoteSelectionPolicy,
    QuoteSelectionResult,
    QuoteSelector,
)
from .reconstruction import ReconstructedTrade, StrategyCycle, TradeReconstructionService
from .replay import BacktestReplayEngine, ReplayInspection, ReplaySnapshot, ReplayStatus
from .scenarios import DEFAULT_SCENARIO_LIBRARY
from .settlement import (
    AssignmentDecision,
    DividendSettlement,
    ExerciseDecision,
    ExpirationProcessingResult,
    PinRiskDiagnostic,
    SettlementEngine,
    SettlementPosting,
    SettlementResult,
)
from .state_machine import (
    ActionName,
    ActionPlan,
    GuardName,
    GuardResult,
    InvalidTransitionError,
    LifecycleState,
    StateTransitionRecord,
    StrategyStateMachine,
    TransitionTrigger,
)
from .strategies import (
    STRATEGY_TEMPLATE_NAMES,
    LegDirection,
    LegEffect,
    LegKind,
    LegSelectionCandidate,
    LegSelectionDiagnostic,
    LegSelectionEngine,
    LegSelectionPolicy,
    LegSelectionResult,
    MultiLegDefinitionLeg,
    MultiLegStrategyDefinition,
    SelectionPolicyType,
    StrategyDefinitionError,
    compile_template,
)
from .strategy_library import (
    AccountType,
    CommonStrategyParameters,
    CompatibilityRules,
    CustomStrategyDefinition,
    EntryRequirements,
    OptimizerParameterContract,
    PayoffPoint,
    PayoffTag,
    RiskClassification,
    RiskLevel,
    StrategyBias,
    StrategyFamily,
    StrategyLibraryError,
    StrategyPayoffAnalyzer,
    StrategyPayoffSummary,
    StrategySelectedLeg,
    StrategyStructuralValidationResult,
    StrategyStructureValidator,
    StrategyTemplate,
    StrategyTemplateCatalogueItem,
    StrategyTemplateLeg,
    StrategyTemplatePluginMetadata,
    StrategyTemplateRegistry,
    StrategyValidationIssue,
    TemplateDeprecation,
    UnderlyingSupport,
    ValidationCode,
    ValidationSeverity,
    classify_strategy_risk,
    compile_strategy_template,
    default_strategy_template_registry,
    deterministic_strategy_library_checksum,
    load_template,
    serialize_template,
    strategy_optimizer_contract,
    strategy_template_api_payload,
)
from .strategy_management import (
    BasisTrackingResult,
    ConversionAnalyticsSummary,
    ConversionPlan,
    EligibilityRejection,
    ExpectedImprovementAssessment,
    ImprovementComponent,
    ManagementAction,
    ManagementComparison,
    ManagementPathAlternative,
    PartialRollState,
    RollAnalyticsSummary,
    RollEligibilityEvaluation,
    RollExecutionIntent,
    RollLegSelection,
    RollLifecycleState,
    RollPolicyDefinition,
    RollPolicyRegistry,
    RollPolicyRegistrySnapshot,
    RollPolicyStatus,
    RollReconciliation,
    RollRequest,
    RollTargetCandidate,
    RollTargetSelector,
    RollType,
    StrategyManagementAnalytics,
    StrategyManagementError,
    StrategyManagementPlanner,
    default_roll_policy_registry,
    deterministic_roll_policy_checksum,
    validate_basis_transfer_invariants,
)
from .strategy_management import (
    RollPlan as StrategyRollPlan,
)
from .strategy_policy_library import (
    PolicyEvaluationContext,
    PolicyEvaluationDiagnostic,
    PolicyEvaluationOutcome,
    PolicyFamily,
    PolicySetEvaluationResult,
    StrategyPolicyAlias,
    StrategyPolicyDefinition,
    StrategyPolicyLibraryError,
    StrategyPolicyRegistry,
    StrategyPolicySet,
    default_strategy_policy_registry,
    deterministic_strategy_policy_checksum,
    policy_set_api_payload,
)
from .transition_guards import GuardEvaluationRequest, TransitionGuardLibrary
from .valuation import ValuationService

__all__ = [
    "ActionName",
    "ActionPlan",
    "ApiEnvelopeV1",
    "ArbitrationDecision",
    "AssignmentDecision",
    "ArbitrationPolicy",
    "AsOfResult",
    "AttributionContractV1",
    "BacktestAnalyticsService",
    "BacktestAsOfQueryService",
    "BacktestBenchmarkResult",
    "BacktestBenchmarkRunner",
    "BacktestConfiguration",
    "BacktestReplayEngine",
    "BacktestRunResult",
    "BacktestStatus",
    "BacktestingEngine",
    "BacktestingError",
    "BaselineResearchFillModel",
    "ClockEvent",
    "ComparisonContractV1",
    "ComparisonResult",
    "ConversionAnalyticsResult",
    "CompetingAction",
    "ConflictMode",
    "ConflictResolutionResult",
    "CoordinatedEntryPlan",
    "CrossStrategyArbitrator",
    "CurveContractV1",
    "DEFAULT_SCENARIO_LIBRARY",
    "DeterministicEvent",
    "default_priority",
    "DrawdownPoint",
    "DividendSettlement",
    "EntryMode",
    "EventClockConfig",
    "EventClockError",
    "EventType",
    "ExecutionAction",
    "ExecutionCalibrationRecord",
    "ExecutionOrderType",
    "ExecutionQualityScore",
    "ExecutionQualityScorer",
    "execution_reproducibility_checksum",
    "execution_calibration_checksum",
    "ExecutionQueryService",
    "ExecutionRequest",
    "ExecutionSide",
    "ExecutionSourceType",
    "ExecutionStressResult",
    "ExecutionStressScenario",
    "ExecutionStressTestEngine",
    "ExerciseAssignmentOrchestrator",
    "ExerciseDecision",
    "ExpirationProcessingResult",
    "FeeItem",
    "FeeModelRequest",
    "FillAttempt",
    "FillExpectation",
    "FillModelConfig",
    "FillModelError",
    "FillModelRequest",
    "FillModelResult",
    "FillPricePolicy",
    "FillQualityAggregate",
    "FillQualityAnalyzer",
    "FillQualityMetrics",
    "FillReconciliationEvent",
    "GuardEvaluationRequest",
    "GuardName",
    "GuardResult",
    "GreeksAttribution",
    "HistoricalEventClock",
    "HistoryContractV1",
    "IntegrityFailure",
    "InvalidTransitionError",
    "LegDirection",
    "LegEffect",
    "LegFillSnapshot",
    "LegKind",
    "LegSelectionCandidate",
    "LegSelectionDiagnostic",
    "LegSelectionEngine",
    "LegSelectionPolicy",
    "LegSelectionResult",
    "LifecyclePolicySignal",
    "LiquidityRegime",
    "LifecycleState",
    "MarginEvaluationSnapshot",
    "MarginLifecycleCoordinator",
    "MarkPricePolicy",
    "MarketRegime",
    "MultiLegExecutionRealism",
    "MultiLegExecutionRealismAnalyzer",
    "ItemizedFeeModel",
    "InteractiveBrokersStyleResearchPolicy",
    "GenericBaselineBrokerPolicy",
    "BrokerPolicyComparisonEngine",
    "BrokerPolicyRegistry",
    "MultiLegDefinitionLeg",
    "MultiLegExecutionCoordinator",
    "MultiLegExecutionResult",
    "MultiLegOrchestrator",
    "MultiLegStrategyDefinition",
    "NoLookAheadError",
    "NoLookAheadGuard",
    "OrderAction",
    "OpenCloseEffect",
    "PartialFillCalibrationResult",
    "PartialFillCalibrator",
    "PlaceholderMarketImpactModel",
    "OrderIntent",
    "OrderSide",
    "OverlayMetrics",
    "PerformanceAnalytics",
    "PnLAttribution",
    "PolicyConflictResolver",
    "PolicyComparisonResult",
    "PolicyDisposition",
    "CalibrationResult",
    "CalibrationValidationRun",
    "CalibrationValidator",
    "PortfolioAnalyticsPoint",
    "PortfolioConflictContractV1",
    "PortfolioSnapshot",
    "PinRiskDiagnostic",
    "QueryAsOfResult",
    "QuotePriceSelection",
    "QuoteSelectionPolicy",
    "QuoteSelectionResult",
    "QuoteSelector",
    "ReconstructedTrade",
    "RealVsSimulatedComparator",
    "RealVsSimulatedFillComparison",
    "ReplayControlContractV1",
    "ReplayRollEventContractV1",
    "ReplayExecutionInspection",
    "ReplayInspection",
    "ReplaySnapshot",
    "ReplaySnapshotContractV1",
    "ReplayStatus",
    "ResearchFillModelEngine",
    "RichEvent",
    "RichEventPriority",
    "RichEventType",
    "RollEligibilityContractV1",
    "RollHistoryContractV1",
    "RollEligibilityResult",
    "RollKind",
    "RollPlan",
    "StrategyRollPlan",
    "RollPlanContractV1",
    "RollPolicyCatalogueContractV1",
    "RollPreviewContractV1",
    "RollRequestContractV1",
    "RollAnalyticsResult",
    "RunSummaryContractV1",
    "StrategyTemplateCatalogueContractV1",
    "StrategyTemplateDetailContractV1",
    "StrategyParameterSchemaContractV1",
    "StrategyValidationContractV1",
    "StrategyPayoffPreviewContractV1",
    "StrategyRiskClassificationContractV1",
    "StrategyOptimizerCompatibilityContractV1",
    "StrategyPolicyCatalogueContractV1",
    "StrategyPolicySetContractV1",
    "StrategyPolicyEvaluationContractV1",
    "StrategyPolicyConflictContractV1",
    "CalendarRollAnalysisContractV1",
    "ConversionPreviewContractV1",
    "ManagementComparisonContractV1",
    "PMCCRollAnalysisContractV1",
    "CustomStrategyCreationContractV1",
    "ScenarioTemplate",
    "SchwabThinkorswimStyleResearchPolicy",
    "SelectionPolicyType",
    "SlippageCalibrator",
    "SlippageModelKind",
    "SpreadCaptureCalibrator",
    "StateTransitionRecord",
    "STRATEGY_TEMPLATE_NAMES",
    "StrategyAnalyticsPoint",
    "StrategyCycle",
    "StrategyCycleContractV1",
    "StrategyDefinitionError",
    "StrategyExecutionProfile",
    "StrategyLifecycle",
    "StrategyLifecycleError",
    "StrategyStateMachine",
    "SettlementEngine",
    "SettlementPosting",
    "SettlementResult",
    "TradeListContractV1",
    "StrategyLibraryError",
    "StrategyFamily",
    "StrategyTemplate",
    "StrategyTemplateLeg",
    "EntryRequirements",
    "CompatibilityRules",
    "RiskClassification",
    "RiskLevel",
    "StrategyBias",
    "OptimizerParameterContract",
    "TemplateDeprecation",
    "StrategyTemplateRegistry",
    "StrategyTemplateCatalogueItem",
    "StrategyTemplatePluginMetadata",
    "StrategySelectedLeg",
    "StrategyValidationIssue",
    "StrategyStructuralValidationResult",
    "StrategyStructureValidator",
    "PayoffPoint",
    "PayoffTag",
    "StrategyPayoffSummary",
    "StrategyPayoffAnalyzer",
    "CommonStrategyParameters",
    "CustomStrategyDefinition",
    "ValidationCode",
    "ValidationSeverity",
    "UnderlyingSupport",
    "AccountType",
    "compile_strategy_template",
    "default_strategy_template_registry",
    "classify_strategy_risk",
    "strategy_optimizer_contract",
    "serialize_template",
    "load_template",
    "strategy_template_api_payload",
    "deterministic_strategy_library_checksum",
    "StrategyPolicyLibraryError",
    "PolicyFamily",
    "StrategyPolicyDefinition",
    "StrategyPolicyAlias",
    "StrategyPolicySet",
    "PolicyEvaluationContext",
    "PolicyEvaluationDiagnostic",
    "PolicyEvaluationOutcome",
    "PolicySetEvaluationResult",
    "StrategyPolicyRegistry",
    "default_strategy_policy_registry",
    "deterministic_strategy_policy_checksum",
    "policy_set_api_payload",
    "StrategyManagementError",
    "RollPolicyStatus",
    "RollLifecycleState",
    "RollType",
    "ManagementAction",
    "RollPolicyDefinition",
    "RollPolicyRegistrySnapshot",
    "RollPolicyRegistry",
    "RollLegSelection",
    "RollRequest",
    "RollTargetCandidate",
    "EligibilityRejection",
    "RollEligibilityEvaluation",
    "ImprovementComponent",
    "ExpectedImprovementAssessment",
    "RollExecutionIntent",
    "PartialRollState",
    "RollReconciliation",
    "BasisTrackingResult",
    "ConversionPlan",
    "ManagementPathAlternative",
    "ManagementComparison",
    "RollAnalyticsSummary",
    "ConversionAnalyticsSummary",
    "RollTargetSelector",
    "StrategyManagementPlanner",
    "StrategyManagementAnalytics",
    "default_roll_policy_registry",
    "deterministic_roll_policy_checksum",
    "TradeReconstructionService",
    "TradingSession",
    "validate_basis_transfer_invariants",
    "TransitionGuardLibrary",
    "TransitionTrigger",
    "TastytradeStyleResearchPolicy",
    "TimeOfDayBucket",
    "TransactionCostBreakdown",
    "TransactionCostEngine",
    "UserDefinedBrokerPolicy",
    "ValuationError",
    "ValuationService",
    "VolatilityRegime",
    "aware_timestamp",
    "BacktestExecutionPolicySelection",
    "classify_liquidity_regime",
    "classify_time_of_day",
    "compile_template",
    "default_execution_stress_scenarios",
    "default_strategy_execution_profiles",
]
